...
type Params struct {
	...
	// RawJSON: if true, output raw JSON from esxi commands without CLI formatting
	// If false, CLI interface will format output as table/text for readability
	RawJSON bool
}
...
func (p *Params) RegisterFlags() {
	...
	flag.BoolVar(&p.RawJSON, "raw-json", false, "Output raw JSON without CLI formatting")
}
